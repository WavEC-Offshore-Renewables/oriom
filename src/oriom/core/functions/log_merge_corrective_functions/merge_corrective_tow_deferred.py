...
class CorrectonDefCreation():
    """
    Class to generate and manage the Correction at Port deferred considering towing, operation, WoW, n_vessels and port spaces

    Attributes:
        operation (: object): object of class ``Operation Major`` at port containing operation metadata and schedules.
        ...
        insp_at_port (: dict): Dictionary storing operation at port operations; keys are device indices, values are tuples (end_datetime, start_datetime).
        operation_completed (: bool): Flag indicating whether the entire operation operation was completed successfully.
        ...
        oper_schedule_insp (: pd.DataFrame): DataFrame containing the operation schedule for operations at port.
    ...
    def __init__(self, operation, n_device_at_port, n_device_stored_at_port, find_element_class, shutdown_col):
        """
        Args:
            operation (: object): object of class ``Operation Major`` at port containing operation metadata and schedules.
            n_device_at_port (: int) Number of devices that can be handled at the port simultaneously.
        ...
        self.tot_device = safe_getattr(operation, ["insp_class", "intervened_devices"])

        # Get the operation schedules
        self.oper_schedule_tow_port = logs_preventive_aux.take_op_schedule_tow(operation = operation, find_element_class = find_element_class, op_tow = 'op_tow_port')
        self.oper_schedule_insp = safe_getattr(operation, ["insp_class", "ts_data", "oper_sched"])
        self.oper_schedule_tow_site_only = logs_preventive_aux.take_op_schedule_tow(operation = operation, find_element_class = find_element_class, op_tow = 'op_tow_site')
        self.oper_schedule_tow_site_port = logs_preventive_aux.take_op_schedule_tow(operation = operation, find_element_class = find_element_class, op_tow = 'op_tow_site_port')

        self.dev_idx_station_port = 1


    def tow_operation_schedule(self, oper_schedule_tow, d, name_op):
        """
        Return end of total op duration and end wait time schedule datetimes for a tow/operation operation starting at a given datetime.
        ...
        try:
            ...
            logging.warning(f'The operation tow {name_op} cannot be completed at date {d}')
    ...
    def overlap_shift_tow(
        ...
        Args:
            ...
            operation (: object): operation object used to read attributes like id and number of vessels.
        ...
        Returns:
            tuple
                ...
                (None, None, None) if a schedule could not be obtained from tow_operation_schedule.
        """

        operation_id = operation.id
        n_vessel = operation.n_vessel_1
        ...
        while overlap_date:
            ...
            for end_2, start_2 in tow_at_site.values():
                ...
                if overlap_date_count >= n_vessel:
                    d_insp, d_tow_port_wait, d_start_tow = self.tow_operation_schedule(self.oper_schedule_tow_port, d_insp, operation_id)
    ...
    def tow_to_port(
        ...
        Call tow_operation_schedule and overlap_shift_tow methods.
        ...
        if device_n <= self.n_device_at_port:
            ...
            d_insp, d_tow_port_wait, d_start_tow_port = self.tow_operation_schedule(self.oper_schedule_tow_port, start_day_op, self.operation.id)
            ...
            if self.tow_at_site:
                ...
                d_insp, d_tow_port_wait, d_start_tow_port = self.overlap_shift_tow(overlap_date, overlap_dict, d_insp, d_tow_port_wait, self.operation, self.n_device_at_port, start_day_op)
                if d_insp is None or d_tow_port_wait is None:
                    self.operation_completed = False
                    return
    ...
    def operation_at_port(
        ...
        """
        Code to inspect the device at port, return the date of end of end operation device at port
        ...
        d_tow, d_insp_wait, _ = self.tow_operation_schedule(self.oper_schedule_insp, d_insp, self.operation.id)
        if d_tow is None or d_insp_wait is None:
            self.operation_completed = False
            return
        self.insp_at_port[self.dev_idx_station_port] = d_tow, d_insp_wait
        # Add shutdown considered in port waiting to start operation
    ...
    def tow_to_site(
        ...
        d_end_device, d_tow_site_wait, d_start_tow_site = self.tow_operation_schedule(oper_schedule_tow_site, d_tow, self.operation.id)
    ...
    def preventive_port_operation(
        ...
        d: datetime,
        df_port_operation_log: pd.DataFrame
    ):
        """
        Main code to evaluate the operation at port for all the devices

        The code evaluate for each device the tow to port, the operation at port and the tow to site.
        ...
        Add to the lists:
            - end_datetimes: end of the operation operation for all the devices
            - end_stat_chart_datetimes: end of the operation operation for statistical analysis
            - valid_datetimes: datetime of start of the operation operation

        Args:
            month_insp (int): Month of the operation
            duration_shutdown_month (list): List with the duration of shutdown for each month
            end_datetimes (list): List with the end datetimes of the operation operations
            end_stat_chart_datetimes (list): List with the end datetimes for statistical analysis
            valid_datetimes (list): List with the valid datetimes of the operation operations
            d (datetime): Datetime of start of the operation operation
            df_port_operation_log (pd.DataFrame): Dataframe to log the towing operations
        """

        actual_df_port_operation_log = pd.DataFrame(columns=df_port_operation_log.columns)
        date_continuous = d
        ...
        for device_n in range(1, self.tot_device+1):
            ...
            actual_df_port_operation_log.loc[len(actual_df_port_operation_log)] = [d, self.tow_at_port[self.dev_idx_station_port][1], self.tow_at_site[self.dev_idx_station_port][0], device_n]
            self.dev_idx_station_port+=1

        # Add the results to the lists if the operation is completed
        if self.operation_completed:
            ...
            df_port_operation_log = pd.concat([df_port_operation_log, actual_df_port_operation_log], ignore_index=True)

        return df_port_operation_log
