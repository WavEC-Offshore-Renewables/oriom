...
def create_logs_corrective_file(
    ...
    for oper_stat in operation_log_file_stats:
        ...
        if getattr(oper, 'tow_to_port', None):
            tow_op_flag = True
            tow_data = TowData.from_operation(find_element_class, oper)
        ...
        for _, row in failure_filter.iterrows():
            ...
            if tow_op_flag:
                ...
                if tow_data.add_op_tow_port:
                    tow_stat_chart_month = tow_data.tow_op_port_stat.dur_total_dict[str(date_failure.month)]
                    op_sched_add_tow_port = safe_getattr(tow_data.add_op_tow_port, ['ts_data','oper_sched'])
                    vessel, ves_2, mob_time = _take_vessel_data(find_element_class = find_element_class, op = tow_data.add_op_tow_port)
                    ...
                    row_add_op_tow_port, row_mob_line_op_tow_port = create_operation_site(
                        ...
                        vessels_ = {'vessel1_id': vessel.id, 'ves_1': tow_data.add_op_tow_port.vessel1_qt, 'vessel2_id': tow_data.add_op_tow_port.vessel2_id, 'ves_2': ves_2},
                        oper_ = {
                            'oper': tow_data.add_op_tow_port, 
                            'oper_stat': tow_data.oper_stat_op_tow_port, 
                        ...
                        index = {'fail_index': fail_index, 'last_valid_idx': safe_getattr(tow_data.add_op_tow_port, ['ts_data','last_valid_index'])},
                ...
                vessel, ves_2, mob_time = _take_vessel_data(find_element_class = find_element_class, op = tow_data.tow_op_port)
                towing_port = CorrectionTowPort(
                    date_failure = date_failure_tow,
                    vessel = vessel,
                    oper = tow_data.tow_op_port,
                    failure = failure,
                ...
                if not deferred_tow:
                    ...
                    towing_port.add_hours_for_noon_shift(
                            ...
                            oper_sched = tow_data.tow_port_op_oper_sched,
                        )
                else:
                    ...
                    index_found = towing_port.check_leadtime_index(oper_sched = tow_data.tow_port_op_oper_sched, CUTOFF_DATE = CUTOFF_DATE)
                    if not index_found:
                        continue
                ...
                df_filtered_start_tow = _check_index_row_validity(
                    ...
                    last_valid_idx= tow_data.last_valid_idx_tow_port,
                    r = row,
                    oper_sched = tow_data.tow_port_op_oper_sched
                )
                ...
                dates_tow_port = compute_operation_datetimes(
                    ...
                    oper_stat = tow_data.tow_op_port_stat,
                    add_op_end = end_add_op_time if tow_data.add_op_tow_port else None,
                ...
                row_tow_port = pd.DataFrame([[
                    ...
                    dates_tow_port['date_end_leadtime'] if not tow_data.add_op_tow_port else date_failure_tow + timedelta(hours=mob_time),
                    ...
                    'tow',
                    tow_data.tow_op_port.id,
                    vessel.id,
                    tow_data.tow_op_port.vessel1_qt,
                    tow_data.tow_op_port.vessel2_id,
                    tow_data.tow_op_port.vessel2_qt,
                    'tow_' + row['id']
                ]],columns=COLS)
            ...
            #------------------------
            if tow_op_flag:
                vessel_tow_site, ves_2, mob_time = _take_vessel_data(find_element_class = find_element_class, op = tow_data.tow_op_site)

                towing_site = CorrectionTowSite(
                    date_failure = date_failure,
                    vessel = vessel_tow_site,
                    oper = tow_data.tow_op_site,
                ...
                index_found = towing_site.check_leadtime_index(oper_sched = tow_data.tow_site_oper_sched, CUTOFF_DATE = CUTOFF_DATE)
                if not index_found:
                    continue
                ...
                df_filtered_start_tow = _check_index_row_validity(
                    ...
                    last_valid_idx= tow_data.last_valid_idx_tow_site,
                    r = row,
                    oper_sched = tow_data.tow_site_oper_sched
                )

                if df_filtered_start_tow.empty:
                    continue

                dates_tow_site = compute_operation_datetimes(df_filtered_start_tow, tow_data.tow_op_site_stat)
                ...
                row_tow_site = pd.DataFrame([[
                    ...
                    'tow',
                    tow_data.tow_op_site.id,
                    vessel_tow_site.id,
                    tow_data.tow_op_site.vessel1_qt,
                    tow_data.tow_op_site.vessel2_id,
                    ves_2,
                    'tow_' + row['id']
                ]],columns=COLS)
                ...
                if tow_data.add_op_tow_site:
                    tow_stat_chart_month = tow_data.tow_op_site_stat.dur_total_dict[str(row_tow_site['d_end_leadtime'].iloc[0].month)]
                    ...
                    fail_index = tow_data.tow_site_oper_sched.index[tow_data.tow_site_oper_sched['datetime'] == end_tow_site_date][0]

                    vessel, ves_2, mob_time = _take_vessel_data(find_element_class = find_element_class, op = tow_data.add_op_tow_site)
                    op_sched_add_tow_site = safe_getattr(tow_data.add_op_tow_site, ['ts_data','oper_sched'])
                    row_add_op_tow_site, row_mob_line_op_tow_site = create_operation_site(
                        ...
                        vessel_ = {'vessel': vessel, 'vessel_to_merge': vessel_to_merge},
                        vessels_ = {'vessel1_id': vessel.id, 'ves_1': tow_data.add_op_tow_site.vessel1_qt, 'vessel2_id': tow_data.add_op_tow_site.vessel2_id, 'ves_2': ves_2},
                        oper_ = {
                            'oper': tow_data.add_op_tow_site,
                            'oper_stat': tow_data.oper_stat_op_site,
                        ...
                        row_ = {'log_events': log_events, 'row': row, 'tow_op_flag': tow_op_flag},
                        index = {'fail_index': fail_index, 'last_valid_idx': safe_getattr(tow_data.add_op_tow_site, ['ts_data','last_valid_index'])},
                        CONST = {'COLS': COLS, 'CUTOFF_DATE': CUTOFF_DATE, 'time_fail_op_immediately': time_fail_op_immediately},
                    )
                    ...
                    if getattr(tow_data.tow_op_site, 'recommisioning_time', None):
                        ...
                        modified_date = row_add_op_tow_site['d_end_dur_net_site'] + timedelta(hours=tow_data.tow_op_site.recommisioning_time)                  
